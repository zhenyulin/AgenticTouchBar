...
Their refresh asks QWeather (和风天气) first, a domestic API that Clash routes
DIRECT, so it keeps answering when the VPN node has timed out. Open-Meteo (no
key) and BTT's `get_weather` (Apple WeatherKit) remain later fallbacks. All
three take a point, and the point is the Mac's own location as BTT reports it,
never a configured coordinate — see
[Where the weather is](#where-the-weather-is). An Apple Weather Shortcut
bridge used to lead this ladder; it is parked (2026-09-18) because a run that
fails raises a Shortcuts alert on every refresh. QWeather needs a free API key
...
BetterTouchTool those three sources stay off, so the row then keeps its last
reading.
