...
class StripVersionAnnotations(unittest.TestCase):
    def test_removes_a_parenthesised_version_marker(self):
        self.assertEqual(strip_version_annotations("Yellow (Remastered 2011)"), "Yellow")
    ...
    def test_dash_suffixed_version_marker_is_not_removed(self):
        # KNOWN GAP: only bracketed markers are stripped, but Apple Music and
        # LRCLIB both commonly use the dash form, so this title still reaches
        # providers with its edition attached. See the report accompanying
        # these tests; normalized() below is unaffected because it keeps only
        # word characters.
        self.assertEqual(
            strip_version_annotations("Yellow - Remastered 2011"),
            "Yellow - Remastered 2011",
        )

    def test_feat_inside_parentheses_leaves_a_dangling_bracket(self):
        # KNOWN WART: the feat clause is removed by a rule that runs after the
        # bracket rule, so the opening bracket survives and "Song (" becomes a
        # search variant. Pinned so a fix is a deliberate, visible change.
        self.assertEqual(strip_version_annotations("Song (feat. X)"), "Song (")
...
class Variants(unittest.TestCase):
    ...
    def test_title_variants_honour_the_search_title_override(self):
        ...
        variants = track_title_variants(track(title="Unconditional", search_title="無條件"))
    ...
    def test_artist_variants_honour_the_search_artist_override(self):
        variants = track_artist_variants(track(artist="Eason Chan", search_artist="陳奕迅"))
